...
# 选择主要的中国公司（包含华为）
chinese_companies = ['Alibaba', 'Huawei', 'ByteDance', 'Tencent', 'Baidu', 'Kylin Software', 'Loongson', 'UnionTech', 'ZTE', 'China Mobile']
...
# 提取主版本号和次版本号，确保正确排序（如6.10 > 6.9）
df_versions[['major', 'minor']] = df_versions['kernelversion'].str.extract(r'(\d+)\.(\d+)').astype(int)
...
# 优化x轴显示
# 只显示部分版本标签，避免重叠
xticks_indices = list(range(0, len(sorted_versions), 3))
xticks_labels = [label.split('(')[0] for label in [sorted_versions[i] for i in xticks_indices]]
plt.xticks(xticks_indices, xticks_labels, fontsize=10, rotation=45, ha='right')
...
# 保存图表
img_path = 'chinese_companies_contribution_with_huawei.png'
